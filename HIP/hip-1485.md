...
making it the standard tool for debugging contract logic, profiling gas, and inspecting internal calls before sending a real transaction. The call object accepts optional `access_list` and `authorization_list` fields as described in [Access List and Authorization List](#access-list-and-authorization-list). Depending on the `tracer` that was provided as an input parameter to the `debug_traceCall`
...
A new requestBody scheme accepting bundles of transactions will be defined. Each transaction object supports the same optional `access_list` and `authorization_list` fields as `debug_traceCall`.
...
The new APIs this HIP introduces does not affect backwards compatibility. Block related APIs will be extended with a new field, which also doesn’t affect backward compatibility. The `/ap1/v1/contracts/call` endpoint will have an extended request body, so all consequent calls should utilize it. The optional `access_list` and `authorization_list` fields on `debug_traceCall`, `debug_traceCallMany`, and `eth_simulateV1` are additive; existing clients that omit them are unaffected.
